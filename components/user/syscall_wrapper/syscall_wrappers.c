...
esp_err_t usr_xTaskSchedule(TaskFunction_t pvTaskCode, char * name){
    TaskHandle_t pvTask;
    usr_task_ctx_t * tskctx = usr_xTaskCreatePinnedToCoreU(pvTaskCode, name, 2048, NULL, 1, &pvTask);
    vTaskSuspend(pvTask);
    EXECUTE_SYSCALL(tskctx, __NR_esp_save_task_ctx);
    return ESP_OK;
}
